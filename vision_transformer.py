...
class VisionTransformer(nn.Module):
    ...
    def match_task_rainbow_style(self, cls_features, cur_id):
        ...
        """
        if not hasattr(self, 'rainbow_prompt') or cls_features is None:
            return 0  # Default to task 0 if no RainbowPrompt or cls_features
        
        base_key = self.rainbow_prompt.base_key
        embed_norm = self.rainbow_prompt.l2_normalize(cls_features, dim=-1)
        matching_result = []
        
        for certain_task in range(cur_id + 1):
            certain_task_key = base_key[certain_task]
            certain_task_key = self.rainbow_prompt.l2_normalize(certain_task_key, dim=-1)
    ...
    def forward(self, x, task_id=-1, learned_id=-1, cls_features=None, train=False, epoch_info=None):
        if not train and cls_features is not None:
            ...
            self.rainbow_prompt.task_id = matched_task_id
        else:
